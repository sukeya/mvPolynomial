  ...
  auto MakeSubIndex(const Array<IntType, D>& index)
  {
    ...
    std::for_each(
      boost::make_zip_iterator(
        boost::make_tuple(projected_index.begin(), index.cbegin())
      ),
      boost::make_zip_iterator(
        boost::make_tuple(projected_index.end(), std::prev(index.cend()))
      ),
      [](const auto& t)
      {
        boost::tuples::get<0>(t) = boost::tuples::get<1>(t);
      }
    );
  ...
  private:
    ...
    )
    {
      ...
      for(auto it = std::next(partitions.begin()); it != partitions.end(); ++it)
      {
        ...
        for(auto partition_it = partition.cbegin(); partition_it != std::prev(partition.cend()); ++partition_it)
        {
          ConstructPartition(
            *it,
            *partition_it,
