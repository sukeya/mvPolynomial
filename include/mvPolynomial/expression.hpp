...
template <class FRef_, class GRef_>
requires is_cref_or_rvalue_v<FRef_> && is_cref_or_rvalue_v<GRef_>
class Composition final {
  using FRef = RefWrapper<FRef_>;
  using GRef = RefWrapper<GRef_>;

 public:
  using F = std::remove_cvref_t<FRef_>;
  using G = std::remove_cvref_t<GRef_>;
  ...
  Composition(FRef_&& f, GRef_&& g) : f_(std::forward<FRef_>(f)), g_(std::forward<GRef_>(g)) {}
...
template <class FRef_>
requires is_cref_or_rvalue_v<FRef_>
class Power {
  using FRef = RefWrapper<FRef_>;
 ...
 public:
  ...
  Power(FRef_&& f, int exp) : f_(std::forward<FRef_>(f)), exp_(exp) {
    if (exp < 0) {
