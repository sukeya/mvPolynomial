...
class Polynomial {
 ...
 public:
  ...
  Polynomial(InputIterator s, InputIterator e) : index2value_(s, e) {
    CheckSelfIndexes();
  }
  ...
  template <typename InputIterator>
  explicit Polynomial(InputIterator s, InputIterator e, const Comparer& c) : index2value_(s, e, c) {
    CheckSelfIndexes();
  }
  ...
  explicit Polynomial(std::initializer_list<value_type> l, const Comparer& c) : index2value_(l, c) {
    CheckSelfIndexes();
  }
  ...
  explicit Polynomial(mapped_type r) { index2value_.at(0) = r; }
  ...
  const mapped_type& operator[](const key_type& index) const {
    return const_cast<Polynomial*>(this)->operator[](index);
  }
  ...
  friend void swap(Polynomial& l, Polynomial& r) { swap(l.index2value_, r.index2value_); }

 private:
  void CheckIndex(key_type index) const {
    if (index < 0) {
      throw std::runtime_error(fmt::format("The index ({}) must be non-negative.", index));
    }
  }
...
template <
    ...
    class AllocatorOrContainer =
        boost::container::new_allocator<std::pair<IndexType<IntType, 1>, R>>>
